...
module.exports = function (socket, options) {
    ...
    if (socket) Socket = socket;else if (o.server) Socket = io.listen(o.server);else throw Error('server or socket should be passed in options!');

    Socket.of(prefix).on('connection', function (socket) {
        var authCheck = options.authCheck;

        if (authCheck && typeof authCheck !== 'function') throw Error('options.authCheck should be function!');

        if (!authCheck) onConnection(options, socket);else authCheck(socket, function () {
...
function onConnection(options, socket) {
    ...
    if (indexEmpty >= 0) ConNum = indexEmpty;else ConNum = Clients.length;

    var msg = log(ConNum + 1, 'console connected\n');
    var dir = WIN ? CWD : tildify(CWD);
...
function cwd(conNum) {
    return function (path) {
        if (!path) return Clients[conNum].cwd;
